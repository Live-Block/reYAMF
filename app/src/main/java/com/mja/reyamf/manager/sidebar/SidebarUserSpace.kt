...
@AndroidEntryPoint
class SidebarUserSpace() : LifecycleService() {
    companion object {
    ...
    override fun onTaskRemoved(rootIntent: Intent) {
        val restartServiceIntent = Intent(applicationContext, SidebarUserSpace::class.java).also {
        ...
        showApp.observe(this) { sidebarApp ->
            YAMFManagerProxy.appList?.filterNotNull()?.let {
                val commonApps = sidebarApp.filter { app1 ->
                    it.any { app2 -> app1.activityInfo.componentName == app2.activityInfo.componentName }
                }.toMutableList()
    ...
    private fun handleSidebar() {
        ...
        binding.ibAppList.setOnClickListener {
            runMain {
                startService(Intent(this@SidebarUserSpace, AppListWindowUserspace::class.java))
            }
            hideMenu()
        }
        ...
        binding.rvSideBarMenu.layoutManager = LinearLayoutManager(this@SidebarUserSpace)
    ...
    private fun startCounter() {
        job = CoroutineScope(Dispatchers.IO).launch {
            delay(200)
            movable = true
            vibratePhone(this@SidebarUserSpace)
        }
    }
        ...
        runOnMainThread {
            if (!isShown) {
                ...
                animateResize(
                    ...
                    75.dpToPx().toInt(), 350.dpToPx().toInt(), this@SidebarUserSpace
                ) {
